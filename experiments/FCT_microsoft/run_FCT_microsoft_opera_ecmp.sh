...
for load in "${load_set[@]}"; do
    for seed in "${seed_set[@]}"; do
        ...
        log_file="../../results/FCT_microsoft/log_"$real_network"_"$workload"_"$load"pload_seed="$seed"_long.txt"
        ...
        ../../src/opera/datacenter/htsim_ndp_dynexpTopology \
            ...
            -flowfile ../../traffic/microsoft/$network/"$workload"_"$load"percLoad_30sec_108N_6hpr_648hosts_"$rate"Gbps_"$active"Nactive_seed="$seed".htsim \
            > "$log_file" &
    done
done      
