  ...
  : Queue(bitrate, maxsize, eventlist, logger)
{
  ...
  _queuesize_high = _queuesize_low = _queuesize_lowest = 0;
  _serv = QUEUE_INVALID;
  stringstream ss;
...
void CompositeQueue::beginService(){
  ...
  if (!_enqueued_high.empty()){
    ...
    _serv = QUEUE_LOW;
    eventlist().sourceIsPendingRel(*this, drainTime(_enqueued_low.back()));
	} else if (!_enqueued_lowest.empty()) {
		_serv = QUEUE_LOWEST;
		eventlist().sourceIsPendingRel(*this, drainTime(_enqueued_lowest.back()));
  }
  else {
    assert(0);
    _serv = QUEUE_INVALID;
...
CompositeQueue::completeService(){
  ...
  if (_serv==QUEUE_LOW){
    ...
    _num_packets++;
	} else if (_serv==QUEUE_LOWEST){
    assert(!_enqueued_lowest.empty());
    pkt = _enqueued_lowest.back();
    _enqueued_lowest.pop_back();
    _queuesize_lowest -= pkt->size();
    _num_packets++;
  ...
  if (!_enqueued_high.empty()||!_enqueued_low.empty()||!_enqueued_lowest.empty())
//   if (!_enqueued_high.empty()||!_enqueued_low.empty())
    beginService();
}
...
CompositeQueue::receivePacket(Packet& pkt)
{
    ...
    if (!pkt.header_only()) {
		...
		if (!pkt.from_large_flow()){
			// cout << "NOT from large flow" << endl;
		    ...
		    if (_queuesize_low + pkt.size() > _maxsize) {
				...
				if (_enqueued_low.empty()){
				    //cout << "QUeuesize_low " << _queuesize_low << " packetsize " << pkt.size() << " maxsize " << _maxsize << endl;
				    assert(0);
				}
		...
		else if (pkt.from_large_flow()) {
			if (_queuesize_lowest+pkt.size() <= _maxsize  || drand()<0.5) {
		    //regular packet; don't drop the arriving packet
		    ...
		    if (_queuesize_lowest + pkt.size() > _maxsize) {
				// we're going to drop an existing packet from the queue
				if (_enqueued_lowest.empty()){
				    // cout << "QUeuesize lowest" << _queuesize_lowest << " packetsize " << pkt.size() << " maxsize " << _maxsize << endl;
				    assert(0);
				}
				//take last packet from low prio queue, make it a header and place it in the high prio queue
				
				Packet* booted_pkt = _enqueued_lowest.front();
				...
				if (booted_pkt->size() >= pkt.size()) {
					...
					_enqueued_lowest.pop_front();
					_queuesize_lowest -= booted_pkt->size();
	    	...
	    	if (chk) {
	    		// the new packet fit, prioritize the booted packet header
			    assert(_queuesize_lowest + pkt.size() <= _maxsize);
			    _enqueued_lowest.push_front(&pkt);
			    _queuesize_lowest += pkt.size();
			    if (_logger) _logger->logQueue(*this, QueueLogger::PKT_ENQUEUE, pkt);
			    if (_serv==QUEUE_INVALID) {
					beginService();
...
CompositeQueue::queuesize() {
    ...
    return _queuesize_lowest + _queuesize_low + _queuesize_high;
}
