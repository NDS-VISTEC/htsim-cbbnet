...
Queue* FatTreeTopology::alloc_src_queue(QueueLogger* queueLogger){
    return  new PriorityQueue(speedFromMbps((uint64_t)_link_rate*8/1000000), memFromPkt(FEEDER_BUFFER), *eventlist, queueLogger);
}

Queue* FatTreeTopology::alloc_queue(QueueLogger* queueLogger, mem_b queuesize){
    return alloc_queue(queueLogger, _link_rate*8/1000000, queuesize);
}
...
void FatTreeTopology::init_network(){
  ...
  for (int k = 0; k < Nup; k++) { // sweep upper pod switch id
    for (int l = 0; l < 6; l++) { // sweep lower pod
      ...
      queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
      logfile->addLogger(*queueLogger);
      queues_nup_nlp[k][j] = alloc_queue(queueLogger, _link_rate*8/1000000, _queuesize);
      ...
      queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
      logfile->addLogger(*queueLogger);
      queues_nlp_nup[j][k] = alloc_queue(queueLogger, _link_rate*8/1000000, _queuesize);
  ...
  for (int j = 0; j < Nup; j++) { // sweep upper pod switch ID
    ...
    for (int l = 0; l < 6; l++) {
      ...
      int k = switch_pos * 6 + l; // get the core switch ID
    	...
    	queues_nup_nc[j][k] = alloc_queue(queueLogger, _link_rate*8/1000000, _queuesize);
    	...
    	//     queues_nc_nup[k][j] = alloc_queue(queueLogger,_link_rate*8/1000000/10, _queuesize);
    	//   cout << "Adding link failure for j" << ntoa(j) << " l " << ntoa(l) << endl;
    	// }
     // 	else
      queues_nc_nup[k][j] = alloc_queue(queueLogger, _link_rate*8/1000000, _queuesize);
...
vector<const Route*>* FatTreeTopology::get_paths(int src, int dest){
  ...
  //Queue* pqueue = new Queue(speedFromMbps((uint64_t)_link_rate*8/1000000), memFromPkt(FEEDER_BUFFER), *eventlist, simplequeuelogger);
  ...
  : EventSource(eventlist,"utilmonitor"), _top(top)
{
    ...
    uint64_t rate = _top->get_link_rate();
    rate = rate * _H;
