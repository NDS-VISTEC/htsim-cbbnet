...
FifoPullQueue<PullPkt>::enqueue(PullPkt& pkt) {
    if (this->_preferred_flow>=0 && static_cast<int>(pkt.flow_id()) == this->_preferred_flow){
	//cout << "Got a pkt from the preffered flow " << pull_pkt->flow_id()<<endl;
	typename list<PullPkt*>::iterator it = _pull_queue.begin();

	while (it!=_pull_queue.end()&& static_cast<int>(((PullPkt*)(*it))->flow_id())==this->_preferred_flow)
	    it++;
    ...
    this->_pull_count++;
    assert(this->_pull_count == static_cast<int>(_pull_queue.size()));
}
...
FifoPullQueue<PullPkt>::flush_flow(int32_t flow_id) {
	...
	PullPkt* pull = *it;
	if (static_cast<int>(pull->flow_id()) == flow_id && pull->type() == NDPPULL) {
	    pull->free();
