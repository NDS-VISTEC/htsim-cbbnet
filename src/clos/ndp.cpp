...
NdpSrc::NdpSrc(NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, int flow_src, int flow_dst)
    : EventSource(eventlist,"ndp"), _flow_src(flow_src), _flow_dst(flow_dst), _logger(logger), _flow(pktlogger)
{
...
void NdpSrc::set_paths(vector<const Route*>* rt_list){
	...
	for (int i=0; i < static_cast<int>(no_of_paths); i++){
...
void NdpSrc::startflow(){
	...
	// cout << "in start flow, sending a packet" << endl;
	send_packet(0);
	_first_window_count++;
    }
}
    ...
    if (cum_ackno >= _flow_size){
	   //cout << "Flow " << nodename() << " finished at " << timeAsMs(eventlist().now()) << endl;
        ...
        cout << "FCT " << get_flow_src() << " " << get_flow_dst() << " " << get_flowsize() <<
            " " << timeAsMs(eventlist().now() - get_start_time()) << " " << timeAsMs(get_start_time()) << " ID " << get_flowid() << endl;
...
void NdpSrc::send_packet(NdpPull::seq_t pacer_no) {
    NdpPacket* p = nullptr;
	// cout << "set if large" << endl;
	// if (_flow_size < 60000000){p->set_from_large_flow(0);}
	// else {p->set_from_large_flow(1);}
	// cout << "end set if large" << endl;

    if (!_rtx_queue.empty()) {
	// There are packets in the RTX queue for us to send
	// cout << "_rtx_queue is not empty" << endl;
	p = _rtx_queue.front();
	...
	if (_flow_size) {
		...
		// cout << "excessive pkt" << endl;
		return;
	    } 
	    if (_highest_sent + _pkt_size >= _flow_size) {
		last_packet = true;
		// cout << "last pkt = true" << endl;
	    }

		// cout << "set if large, pacer no = " << pacer_no << endl;
		// cout << "flow size = " << _flow_size << endl;
    ...
    if (_flow_size < 60000000) {
		p->set_from_large_flow(0);}
	else {
		p->set_from_large_flow(1);}
...
{
    // _packet_drain_time = (simtime_picosec)(Packet::data_packet_size() * (pow(10.0,12.0) * 8) / (uint64_t)_pull_rate*8)/pull_rate_modifier;
    // _packet_drain_time = (simtime_picosec)(Packet::data_packet_size() * (pow(10.0,12.0) * 8) / speedFromMbps((uint64_t)40000))/pull_rate_modifier;HOST_NIC
    ...
    : EventSource(event, "ndp_pacer"), _last_pull(0)
{
    ...
    if (!_pull_spacing_cdf) {
        ...
        if (fscanf(f, "%d\n", &_pull_spacing_cdf_count) != 1) {
            cerr << "Error: Failed to read pull spacing CDF count from " << filename << endl;
            fclose(f);
            exit(EXIT_FAILURE);
        }

        cout << "Generating pull spacing from CDF; reading " << _pull_spacing_cdf_count
        ...
        for (int i = 0; i < _pull_spacing_cdf_count; i++) {
            if (fscanf(f, "%d %lf\n", &t, &_pull_spacing_cdf[i]) != 2) {
                cerr << "Error: Failed to read entry " << i << " from " << filename << endl;
                delete[] _pull_spacing_cdf; // Clean up allocated memory
                fclose(f);
                exit(EXIT_FAILURE);
            }
        }

        fclose(f); // Close the file after processing
    }
