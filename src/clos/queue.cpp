...
PriorityQueue::getPriority(Packet& pkt) {
    // cout << "getting priority, type = " << pkt.type() << endl;
    queue_priority_t prio = Q_LOWEST;
    switch (pkt.type()) {
    case TCPACK:
    case NDPACK:
    case NDPNACK:
	...
	if (pkt.header_only()) {
	    prio = Q_HI;
        // cout << "header_only, set prio = Q_HI" << endl;
	} else {
	    NdpPacket* np = (NdpPacket*)(&pkt);
	    if (np->retransmitted()) {
		prio = Q_MID;
        // cout << "retransmitted, set prio = Q_MID" << endl;
	    } else {
		prio = Q_LO;
        if (np->from_large_flow()) 
            prio = Q_LOWEST;
	    }
	}
	break;
...
PriorityQueue::serviceTime(Packet& pkt) {
    ...
    switch (prio) {
    case Q_LOWEST:
	// cout << "q_lowest: " << _queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO] + _queuesize[Q_LOWEST] << " ";
	// cout << "q_lowest: " << _queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO] + _queuesize[Q_LOWEST] << " = " << _queuesize[Q_HI] << " + " << _queuesize[Q_MID] << " + " << _queuesize[Q_LO] << " + "  << _queuesize[Q_LOWEST] << endl;
	return (_queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO] + _queuesize[Q_LOWEST]) * _ps_per_byte;
    case Q_LO:
	//cout << "q_lo: " << _queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO] << " = " << _queuesize[Q_HI] << " + " << _queuesize[Q_MID] << " + " << _queuesize[Q_LO] << endl;
	return (_queuesize[Q_HI] + _queuesize[Q_MID] + _queuesize[Q_LO]) * _ps_per_byte;
    case Q_MID:
	// cout << "q_mid: " <<_queuesize[Q_HI] + _queuesize[Q_MID] << " = " << _queuesize[Q_HI] << " + " << _queuesize[Q_MID] << endl;
	return (_queuesize[Q_HI] + _queuesize[Q_MID]) * _ps_per_byte;
    case Q_HI:
	// cout << "q_hi: " <<  _queuesize[Q_HI] << " = " << _queuesize[Q_HI] << endl;
...
PriorityQueue::receivePacket(Packet& pkt) 
{
	...
	/* schedule the dequeue event */
	assert(_queue[Q_LOWEST].size() + _queue[Q_LO].size() + _queue[Q_MID].size() + _queue[Q_HI].size() == 1);
	beginService();
    }
}
...
{   
    ...
    // cout << "END completing service" << endl;
}

mem_b
PriorityQueue::queuesize() {
    return _queuesize[Q_LOWEST] + _queuesize[Q_LO] + _queuesize[Q_MID] + _queuesize[Q_HI];
}
