...
class NdpSrc : public PacketSink, public EventSource {
 ...
 public:
    NdpSrc(networkDevices* net, dynamicTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, int flow_src, int flow_dst);
    ...
    inline uint64_t get_flowid() {return _flow_id;} // bytes
...
class NdpSink : public PacketSink, public DataReceiver, public Logged {
 ...
 public:
    ...
    NdpSink(networkDevices* net, dynamicTopology* top, NdpPullPacer* pacer, int flow_src, int flow_dst);
...
class NdpPullPacer : public EventSource {
 public:
    ...
    void set_preferred_flow(int id) { _preferred_flow = id; if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Preferring flow "<< id << endl;};

    void sendToNIC(Packet* pkt);
    void set_pull_rate(uint64_t link_rate, double pull_rate_modifier) {
        _packet_drain_time = (simtime_picosec)(Packet::data_packet_size() * (1000000000000 * 8) / speedFromMbps((uint64_t)link_rate*8/1000000))/pull_rate_modifier;
    }
 private:
