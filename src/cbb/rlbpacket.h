...
class RlbPacket : public Packet {
 public:
    ...
    inline static RlbPacket* newpkt(networkDevices* net, dynamicTopology* top, PacketFlow &flow, int src, int dst, RlbSink* rlbsink, int size, int seqno) {
    ...
    virtual inline void  strip_payload() {
	   Packet::strip_payload(); _size = HEADER;
        cbb_logging::error() << "Sripping payload of RLB packet - shouldn't happen!" << endl;
    };
