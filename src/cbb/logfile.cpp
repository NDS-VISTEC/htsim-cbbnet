...
Logfile::Logfile(const string& filename, EventList& eventlist, int type, networkDevices* net, dynamicTopology* top, string sys) 
: EventSource(eventlist,"logfile"), _starttime(0), _eventlist(eventlist), _preamble(ios_base::out | ios_base::in) {
    _numRecords = 0;
    _logfilename = filename;
    logType = type;
    _queuefilename = filename;
    netDev = net;
    dynTop = top;
    systemName = sys;
...
void Logfile::writeLogHeader() {
    ...
    ofstream log;
    log.open(_logfilename, ios::out | ios::trunc);
    //cout << "Printing header\n"; // For debugging
    
    log << "SRC,DST,FLOWSIZE (BYTE),DURATION (MS),STARTTIME (MS)\n";
        
    log.close();
}

void Logfile::writeQueueHeader() {
    ...
    log.close();
    eventlist().sourceIsPendingRel(*this, dynTop->get_slicetime(3));
}

void Logfile::writeQueue() {

    
    
    ofstream log;
    log.open(_queuefilename, ios::out | ios::app);

    log << "Current time = " << timeAsMs(eventlist().now()) << " ms." << endl;

    log.close();
    
}

void Logfile::writeLogFCT(int src, int dst, int flowSize, double duration, double startTime) {

    ofstream log;
    log.open(_logfilename, ios::out | ios::app);

    string result = to_string(src) + "," + to_string(dst) + "," + to_string(flowSize) +
            "," + to_string(duration) + "," + to_string(startTime) + "\n";
    //cout << result << endl; // For debugging
