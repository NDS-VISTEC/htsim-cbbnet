...
class TcpSrc : public PacketSink, public EventSource {
 ...
 public:
    TcpSrc(networkDevices* net, dynamicTopology* top, EventList &eventlist, int flow_src, int flow_dst, Logfile* logfile);
    ...
    inline uint64_t get_flowsize() {return _flow_size;} 
...
class TcpSink : public PacketSink, public DataReceiver, public Logged {
    friend class TcpSrc;
 public:
    TcpSink(networkDevices* net, dynamicTopology* top, int flow_src, int flow_dst);
