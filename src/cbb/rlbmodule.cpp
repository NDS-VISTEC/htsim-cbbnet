...
RlbModule::RlbModule(networkDevices* net, dynamicTopology* top, EventList &eventlist, int node)
  : EventSource(eventlist,"rlbmodule"), _top(top), _net(net), _node(node)
{
    _H = _top->no_of_nodes(); // number of hosts

    _F = 1; // Tuning parameter: scales back how much RLB traffic to send
    //cout << _top->get_slicetime(3) <<" Help\n";
    // _Ncommit_queues = _top->no_of_hpr(); // number of commit queues = number of hosts per rack
    _Ncommit_queues = _top->no_of_upl(); // number of commit queues = number of hosts per rack

    _mss = 1436; // packet payload length (bytes)
    _hdr = 64; // header length (bytes)
    _link_rate = net->get_link_rate();

    // _slot_time = timeAsSec((_Ncommit_queues - 1) * (double)_top->get_slicetime(3) + (double)_top->get_slicetime(0)); // seconds (was 0.000281;)
    ...
    _max_send_rate = (_F / _top->no_of_hpr()) * _good_rate;

    //_max_send_rate = _good_rate; // DEBUG: DISABLE BANDWIDTH LIMIT
    ...
    // For DA.
    _rlb_queues_da.resize(2); // allocate
    for (int i = 0; i < 2; i++)
        _rlb_queues_da[i].resize(_H);

    _rlb_queue_sizes_da.resize(2); // allocate & init
    for (int i = 0; i < 2; i++) {
        _rlb_queue_sizes_da[i].resize(_H);
        for (int j = 0; j < _H; j++)
            _rlb_queue_sizes_da[i][j] = 0;
    }
...
void RlbModule::doNextEvent() {
    // cout << "Module doing next event." << endl;
...
void RlbModule::receivePacket(Packet& pkt, int flag)
{
    ...
    if (pkt.get_current_local_src() == _node) { 
        ...
        if (pkt.get_dst() == _node) {
            ...
            if (pkt.get_src() == _node) {
                ...
                cbb_logging::error() << "RLB packet received at wrong host: get_dst() = " << pkt.get_dst() << ", _node = " << _node << endl;
                cbb_logging::error() << "... get_real_dst() = " << pkt.get_real_dst() << ", get_real_src() = " << pkt.get_real_src() << ", get_src() = " << pkt.get_src() << endl;
                abort();
            }
        }
    }
...
Packet* RlbModule::NICpull()
{
    ...
    // debug:
    //cout << "   Rlbmodule[node" << _node << "] - the NIC pulled a packet at " << timeAsUs(eventlist().now()) << " us. in ul: "<< _pull_cnt << endl;
    ...
    while (!found_queue) {
        if (!_commit_queues[_pull_cnt].empty()) {
            ...
            pkt->set_commit_port(_pull_cnt + _top->no_of_hpr());
            ...
            if (!pkt->is_dummy()) {
                ...
                found_queue = true;
                //cout << "current pull = " << _pull_cnt << endl;
                
                // DISABLED FOR NOW.
                /*
                bool found = false;
                int limit = _Ncommit_queues;
                while (!found && limit >= 0) {
                    _pull_cnt++;
                    _pull_cnt = _pull_cnt % _Ncommit_queues;
                    if (!_commit_queues[_pull_cnt].empty()) {
                        if (!((_commit_queues[_pull_cnt].front())->is_dummy())) {
                            found = true;
                            break;
                        }
                        else {
                        ...
                        //cout << "FINDING PULL_CNT with limit = " << limit << endl;;
                        limit -= 1;
                    }
                }
                //cout << "EXIT with new _pull_cnt = " << _pull_cnt << endl;
                found_queue = true;*/
...
void RlbModule::check_all_empty()
{
    ...
    if (!any_have) {
        ...
        PriorityQueue* nic = dynamic_cast<PriorityQueue*>(_net->get_queue_serv_tor(_node));
        nic->doorbell(false);
    }
}
    ...
    for (int i = 0; i < num_to_push; i++) {
        if (!_commit_queues[_push_cnt].empty()) {
            ...
            if (!pkt->is_dummy()) { // it's a "real" packet
                ...
                if(pkt->get_current_local_src() == _node) { // it's a local packet
...
void RlbModule::enqueue_commit(int slice, int current_commit_queue, int Nsenders, const vector<int>& pkts_to_send, const vector<vector<int>>& q_inds, const vector<int>& dst_labels)
{

    // convert #packets_to_be_sent into sending_rates:
    // cout << "\n*** New round of enqueue with Nsenders = " << Nsenders << " | " << _node << endl;
    //int all_pkts = 0;
    vector<double> sending_rates;
    for (int i = 0; i < Nsenders; i++) {
        sending_rates.push_back(pkts_to_send[i] / _slot_time);
    }
    //  cout << "max pkts = " << _max_pkts << endl;
    // debug:
    ...
    for (int i = 0; i < Nsenders; i++) {
        ...
        while (time < _slot_time) {
            //cout << i << " | Time = " << time << ". Size of send_times = " << send_times[i].size() << " and Nsenders = " << Nsenders << endl;
    ...
    //cout << "The size of send_times[0] = " << send_times[0].size() << endl;
    ...
    for (int i = 0; i < _max_pkts; i++){
        ...
        while (!found_queue) {
            ...
            double end_slot_time = (i + 1) * (_top->no_of_hpr() * _pkt_ser_time); // send_time must be less than this to send in this slot
            ...
            //fst_sndr = sender;
            //sender +=1 ;
            //sender = sender % Nsenders;

            //cout << "fst_sndr = " << fst_sndr << endl;
            if (fst_sndr != -1) { // there was a queue that can send
                // now check if the queue has packets

                //cout << "RLB Q size of " << fst_sndr << " = " << _rlb_queue_sizes[q_inds[0][fst_sndr]][q_inds[1][fst_sndr]] << endl;
                if (_rlb_queue_sizes[q_inds[0][fst_sndr]][q_inds[1][fst_sndr]] > 0) {
                    ...
                    // debug:
                    //cout << " Rlbmodule - committing from queue[" << q_inds[0][fst_sndr] << "][" << q_inds[1][fst_sndr] << "], changing destination to " << dst_labels[fst_sndr] << endl;
                    ...
                    //cout << "Commit size of " << current_commit_queue << " = " << _commit_queues[current_commit_queue].size() << endl;
                ...
                //cout << "Passing to the new packet selection\n";
...
void RlbModule::clean_commit()
{
    ...
    // debug:
    //cout << "Cleaning commit queue[" << _node << "][" << oldest << "] at " << timeAsUs(eventlist().now()) << " us. with size = " << _commit_queues[oldest].size() << endl;
...
void RlbModule::moveToLocal(int host_to_move) {

    // if (!_rlb_queues[0][host_to_move].empty()) {cout << "Q at host " << _node << " to host " << host_to_move << " has size " << _rlb_queues[0][host_to_move].size() << endl; }

    while (!_rlb_queues[0][host_to_move].empty()) {
        
        _rlb_queues[0][host_to_move].back()->set_current_local_src(_node);
        _rlb_queues[1][host_to_move].push_front(_rlb_queues[0][host_to_move].back());
        _rlb_queues[0][host_to_move].pop_back();

        _rlb_queue_sizes[1][host_to_move]++;
        _rlb_queue_sizes[0][host_to_move]--;
    }
...
RlbMaster::RlbMaster(networkDevices* net, dynamicTopology* top, EventList &eventlist, int val, int interval)
  : EventSource(eventlist,"rlbmaster"), _top(top), _net(net), _modified(val), _interval(interval)
{
...
void RlbMaster::doNextEvent() {
    // cout << "Master doing next event." << endl;
...
void RlbMaster::newMatching(){
    int _moveLocal = _top->getMoveLocal();

    _moveLocal = 1;
    if (_moveLocal) {
        for (int inter = 0; inter < _N; inter++) {
            for (int dst = 0; dst < _N; dst++) {
                if (inter != dst && _top->get_emulated_connectivity(inter, dst) == 0) {
                    for (int int_host = 0; int_host < _hpr; int_host++) {
                        RlbModule* module = _net->get_rlb_module((inter*_hpr) + int_host);
                        for (int dst_host = 0; dst_host < _hpr; dst_host++) {
                            module->moveToLocal((dst*_hpr) + dst_host);
                        }
    ...
    int slice = 0 + (superslice * 3);


    // copy queue sizes from each host into a datastructure that covers all hosts
    RlbModule* mod;
    for (int host = 0; host < _H; host++) {
        mod = _net->get_rlb_module(host); // get pointer to module
        const vector<vector<int>>& _queue_sizes_temp = mod->get_queue_sizes(); // get queue sizes from module
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < _H; k++) {
    ...
    for (int i = 0; i < _H; i++) {
        ...
        _link_caps_send[i] = _max_pkts*_nul/_hpr;
        _link_caps_recv[i] = _max_pkts*_nul/_hpr;
    ...
    for (int crtToR = 0; crtToR < _N; crtToR++) {
        ...
        if (crtToR != dstToR) {
            ...
            phase2(src_hosts, dst_hosts, slice); // the dst_hosts compute how much to accept from the src_hosts
    ...
    for (int crtToR = 0; crtToR < _N; crtToR++) {
        ...
        if (crtToR != dstToR) {
            ...
            for (int j = 0; j < (int)src_hosts.size(); j++){
                for (int k = 0; k < (int)dst_hosts.size(); k++) {
                ...
                if (_Nsenders[src_hosts[j]] > 0) {
                    mod = _net->get_rlb_module(src_hosts[j]);
    ...
    _current_commit_queue = _current_commit_queue % _nul;
    // --------- set up next new matching --------- //
...
void RlbMaster::phase1(const vector<int>& src_hosts, const vector<int>& dst_hosts)
{
    ...
    // LEGACY FIXED_PKTS (OUTDATED)
    int fixed_pkts = _max_pkts / _hpr; // maximum "safe" number of packets we can send

    for (int i = 0; i < (int)src_hosts.size(); i++) { // sweep senders

        _Nsenders[src_hosts[i]] = 0; // initialize number of sending queues to zero
    
        for (int j = 0; j < (int)dst_hosts.size(); j++) { // sweep destinations
            
            int temp_pkts;
            
            // Check that system is considering normal or very large flows (DA). 
            temp_pkts = _working_queue_sizes[src_hosts[i]][0][dst_hosts[j]]; // 0 = nonlocal
            ...
            if (temp_pkts > 0) {
                ...
                //cout << "RlbMaster - src: " << src_hosts[i] << ", dst: " << dst_hosts[j] << endl;
                //cout << "   send " << _pkts_to_send[src_hosts[i]].back() << " packets from nonlocal queue"<< endl;
    ...
    for (int i = 0; i < (int)src_hosts.size(); i++)
        local_pkts_rack[i].resize(dst_hosts.size());

    for (int i = 0; i < (int)src_hosts.size(); i++) {
        for (int j = 0; j < (int)dst_hosts.size(); j++) {
    ...
    // get vectors of the capacities:
    vector<int> dst_caps;
    for (int i = 0; i < (int)dst_hosts.size(); i++)
    ...
    for (int i = 0; i < (int)src_hosts.size(); i++) {
        ...
        temp = _working_queue_sizes[src_hosts[i]][1];
...
void RlbMaster::phase2(const vector<int>& src_hosts, const vector<int>& dst_hosts, int slice)
{
    ...
    int fixed_pkts;
    if (_hpr == 1) {fixed_pkts = _max_pkts / _hpr;}
    else {fixed_pkts = 1 * _max_pkts / _hpr;}

    for (int i = 0; i < (int)dst_hosts.size(); i++) { // sweep destinations

        vector<int> temp;
        temp = _working_queue_sizes[dst_hosts[i]][0];
        for (int j = 0; j < _H; j++) {
        ...
        for (int j = 0; j < (int)src_hosts.size(); j++) {   
            for (int k = 0; k < _H; k++) {
                if (k != src_hosts[j]) {
                    ...
                    int connected = _top->get_emulated_connectivity(dst_hosts[i]/_hpr, k/_hpr);
                    
                    if (!connected) {
                        _proposals[src_hosts[j]][k] = 0; // Set offer to zero
                    }
                }
            }          
...
vector<vector<int>> RlbMaster::fairshare2d(vector<vector<int>> input, const vector<int>& cap0, const vector<int>& cap1) {
...
vector<vector<int>> RlbMaster::fairshare2d_2(vector<vector<int>> input, int cap0, const vector<int>& cap1) {
