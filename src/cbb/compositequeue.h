...
class CompositeQueue : public Queue {
 public:
    CompositeQueue(linkspeed_bps bitrate, mem_b maxsize, 
		   EventList &eventlist, int tor, int port, dynamicTopology* top);
    virtual void receivePacket(Packet& pkt);
    virtual void doNextEvent();
    // should really be private, but loggers want to see
    mem_b _queuesize_rlb, _queuesize_low, _queuesize_high, _queuesize_cbb_cnt;
 ...
 protected:
    ...
    list<Packet*> _enqueued_cbb_cnt; // control packet queue
};
