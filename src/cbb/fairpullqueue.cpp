...
FifoPullQueue<PullPkt>::enqueue(PullPkt& pkt) {
    if (this->_preferred_flow>=0 && (int32_t)pkt.flow_id() == this->_preferred_flow){
	//cout << "Got a pkt from the preffered flow " << pull_pkt->flow_id()<<endl;
	typename list<PullPkt*>::iterator it = _pull_queue.begin();

	while (it!=_pull_queue.end()&& (int32_t)((PullPkt*)(*it))->flow_id()==this->_preferred_flow)
	    it++;

	_pull_queue.insert(it, &pkt);
    } else {
	if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Enqueue";
	_pull_queue.push_front(&pkt);
    }
    this->_pull_count++;
    assert(this->_pull_count == (int)_pull_queue.size());
}
...
FifoPullQueue<PullPkt>::dequeue() {
	...
	if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Dequeue empty";
	return 0;
    }
    if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Dequeue";
...
FifoPullQueue<PullPkt>::flush_flow(int32_t flow_id) {
	...
	PullPkt* pull = *it;
	if ((int32_t)pull->flow_id() == flow_id && pull->type() == NDPPULL) {
	    pull->free();
