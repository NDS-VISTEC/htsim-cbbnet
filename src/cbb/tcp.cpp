...
TcpSrc::TcpSrc(networkDevices* net, dynamicTopology* top, EventList &eventlist, int flow_src, int flow_dst, Logfile* logfile)
    : EventSource(eventlist,"tcp"), _logfile(logfile), _flow(NULL) {
...
void 
TcpSrc::connect(TcpSink& sink, simtime_picosec starttime) {
    //_route = &routeout;
...
TcpSrc::receivePacket(Packet& pkt) 
{
    ...
    if (seqno < _last_acked) {
		cbb_logging::error() << "O seqno" << seqno << " last acked "<< _last_acked;
		return;
    }
    ...
    if (seqno==1){
		...
		cbb_logging::error() << "Should be _established " << seqno << endl;
    }
	...
	//cout << "Src has seqno:" << seqno << " and flowsize, lastAcked, highestSent = " << _flow_size << " " << _last_acked<< " " << _highest_sent<< endl;
    if (seqno >= _flow_size && finished == 0){
		//cout << "Flow " << get_flow_src() << " to " << get_flow_dst() << " finished at " << timeAsMs(eventlist().now()) << endl;	
		cout << "FCT " << get_flow_src() << " " << get_flow_dst() << " " << get_flowsize() <<
            " " << timeAsMs(eventlist().now() - get_start_time()) << " " << fixed << timeAsMs(get_start_time()) << endl;
		finished = 1;
		//cout << _logfile << endl;
		if (_logfile != NULL) {
			_logfile->writeLogFCT(get_flow_src(), get_flow_dst(),get_flowsize(), timeAsMs(eventlist().now() - get_start_time()), timeAsMs(get_start_time()));
		}
    ...
    if (seqno > _last_acked) { // a brand new ack
		...
		//printf("Host %d received ACK from %d\n", get_flow_src(), get_flow_dst());
		...
		if (!_in_fast_recovery) { // best behaviour: proper ack of a new packet, when we were expecting it
			...
			//printf("	receivePacket:: Not in fast recovery with _last_acked = %d\n", _last_acked);
		...
		//	_logger->logTcp(*this, TcpLogger::TCP_RCV_FR);   
			if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Retransmit\n";
		retransmit_packet();
		send_packets();
		return;
    }
...
void TcpSrc::send_packets() {
    ...
    if (!_established) {
		...
		TcpPacket * p  = TcpPacket::new_syn_pkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, 1, 1);
		_highest_sent = 1;
	...
	//cout << "Cond1 = " << (_last_acked + c >= _highest_sent + _mss) << " and Cond2 = " << (_highest_sent+_mss <= _flow_size+1) << endl;
	//cout << "highest_sent = " << _highest_sent << " and mss = " << _mss << endl;
	//cout << "Cwnd = " << _cwnd << endl;

    while ((_last_acked + c >= _highest_sent + _mss) && (_highest_sent + _mss <= _flow_size+_mss+1)) { // Delete _mss
		uint64_t data_seq = 0;
		/*
		#ifdef MODEL_RECEIVE_WINDOW
		...
		#else
			TcpPacket* p = TcpPacket::newpkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, _highest_sent+1, data_seq, _mss);
		#endif */
		TcpPacket* p = TcpPacket::newpkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, _highest_sent+1, data_seq, _mss);
...
void TcpSrc::retransmit_packet() {
	...
	cbb_logging::error() << "Failed to find packet on " << _flow.id << " last_acked " << _last_acked+1 << " highest_sent " << _highest_sent << endl;
	assert(NULL);
    }
    //  else
...
#else
    TcpPacket* p = TcpPacket::newpkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, _last_acked+1, data_seq, _mss);
#endif
...
void TcpSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    if (cbb_logging::verbose_enabled()) cbb_logging::debug() <<"At " << now/(double)1000000000<< " RTO " << _rto/1000000000 << " MDEV " 
...
void TcpSrc::doNextEvent() {
	...
	if(_rtx_timeout_pending) {
		...
		if (_in_fast_recovery) {
	    	...
	    	_cwnd = min(_ssthresh, flightsize); // Delete mss
		}
...
TcpSink::TcpSink(networkDevices* net, dynamicTopology* top, int flow_src, int flow_dst) 
    : Logged("sink"), _cumulative_ack(0) , _packets(0), _mSink(0) {
...
void TcpSink::send_ack(simtime_picosec ts,bool marked) {
    ...
    TcpAck *ack = TcpAck::newpkt(_net, _top, _src->_flow, _flow_dst, _flow_src, _src, 0, _cumulative_ack, (_mSink!=NULL?_mSink->data_ack():0));
	//cout << "Create ACK from snk:" << _flow_dst <<" to src:" << _flow_src<<endl;
