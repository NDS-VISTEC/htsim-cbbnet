...
int MultipathTcpSrc::getDataSeq(uint64_t* seq, TcpSrc* subflow){
  ...
  if (_last_acked+_receive_window > _highest_sent){
	...
	*seq = packet;
    ...
    cbb_logging::error() << "Fail Data Seq" << endl;
...
MultipathTcpSrc::inflate_window(uint32_t cwnd, int newly_acked,uint32_t mss) {
  ...
  case COUPLED_INC:
    ...
    if (tmp < 0){
      fprintf(stderr, "Negative increase!");
      tmp = 0;
    }
  ...
  default:
    fprintf(stderr, "Unknown cc type %d\n",_cc_type);
    exit(1);
  }
}
...
MultipathTcpSrc::compute_a_tcp(){
  ...
  if (_subflows.size()!=2){
    cbb_logging::error() << "Expecting 2 subflows, found" << _subflows.size() << endl;
    exit(1);
  }
...
uint32_t MultipathTcpSrc::compute_a_scaled(){
  ...
  if (alpha==0){
    cbb_logging::error() << "alpha is 0 and t is "<<t <<" cwndSum "<<cwndSum<<endl;
    alpha = A_SCALE;
  }
...
MultipathTcpSrc::compute_a(){
  ...
  if (_cc_type==COUPLED_INC){
    ...
    if (a<0.5){
      cbb_logging::error() << " a comp error " << a << ";resetting to 0.5" <<endl;
      a = 0.5;
    }
...
void MultipathTcpSink::doNextEvent(){
  ...
  if (_last_min_rtt<timeFromMs(1))
    cbb_logging::error() << "Problem with last min rtt "<<endl;
  else{
