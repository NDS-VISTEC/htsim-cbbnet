...
class Pipe : public EventSource, public PacketSink {
 public:
    ...
    uint64_t reportBytesTotal(); // reports to the UtilMonitor
    uint64_t _bytes_delivered; // keep track of how many (non-hdr,ACK,NACK,PULL,RTX) packets were delivered to hosts
    uint64_t _bytes_delivered_total; // keep track of how many (non-hdr,ACK,NACK,PULL,RTX) packets were delivered to hosts
...
class UtilMonitor : public EventSource {
 public:
    ...
    UtilMonitor(dynamicTopology* top, networkDevices* net, EventList &eventlist);
