...
NdpSrc::NdpSrc(networkDevices* net, dynamicTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, int flow_src, int flow_dst)
    : EventSource(eventlist,"ndp"), _flow(pktlogger){
...
void NdpSrc::log_me() {
    ...
    if (_log_me == true)
        return;
    if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Enabling logging on NdpSrc " << _nodename << endl;
    _log_me = true;
    if (_sink)
        _sink->log_me();
}
...
void NdpSrc::startflow() {
    ...
    if (cbb_logging::verbose_enabled() && get_flowid() == 3469) {cbb_logging::debug() << "flow " << get_flowid() << " has started, at " << eventlist().now() << endl;}
    while (_flight_size < _cwnd && _flight_size < _flow_size) {
        if (cbb_logging::verbose_enabled() && get_flowid() == 3469) {cbb_logging::debug() << "flow " << get_flowid() << " sending a packet, at " << eventlist().now() << endl;}
        send_packet(0);
        _first_window_count++;
    }
}
...
#define SWALLOW
	...
	//for a pull.  Waiting reduces the effective window by one.
	   ...
	   send_packet(0);
		   if (_log_me && cbb_logging::verbose_enabled()) {
		       cbb_logging::debug() << "    bounce send pw=" << _pull_window << endl;
	   } else {
	       //printf("bounce send\n");
	   }
    } else {
        ...
        //cout << "    bounce swallow" << endl;
		   if (_log_me && cbb_logging::verbose_enabled()) {
		       cbb_logging::debug() << "    bounce swallow pw=" << _pull_window << endl;
	   } else {
...
void NdpSrc::processNack(const NdpNack& nack){
    ...
    p = NdpPacket::newpkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, nack.ackno(),
    ...
    if (cum_ackno >= _flow_size){
	   //cout << "Flow " << nodename() << " finished at " << timeAsMs(eventlist().now()) << endl;
        ...
        cout << "FCT " << get_flow_src() << " " << get_flow_dst() << " " << get_flowsize() <<
            " " << timeAsMs(eventlist().now() - get_start_time()) << " " << fixed << timeAsMs(get_start_time()) << " ID " << get_flowid() << endl;
...
void NdpSrc::receivePacket(Packet& pkt) 
{
    ...
    case NDPACK:
	{
        ...
        if (cbb_logging::verbose_enabled() && pkt.get_ndpsrc()->get_flowid() == 3469) {cbb_logging::debug() << "src received an ACK, at " << eventlist().now() << endl;}
...
Queue* NdpSrc::sendToNIC(NdpPacket* pkt) {
    ...
    // Queue* nic = net->get_queue_serv_tor(pkt->get_src());
    PriorityQueue* nic = (PriorityQueue*)net->get_queue_serv_tor(pkt->get_src()); // returns pointer to nic queue
...
void NdpSrc::send_packet(NdpPull::seq_t pacer_no) {
    ...
    if (!_rtx_queue.empty()) {
        ...
        p = NdpPacket::newpkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, _highest_sent+1,
        ...
        //PacketSink* sink = p->sendOn();
        //PacketSink* sink = p->sendToNIC();
        if (cbb_logging::verbose_enabled() && p->get_ndpsrc()->get_flowid() == 3469) {cbb_logging::debug() << "sending a packet to NIC, at " << eventlist().now() << endl;}
...
void NdpSrc::retransmit_packet() {
    ...
    for (j = rtx_list.begin(); j != rtx_list.end(); j++) {
        ...
        p = NdpPacket::newpkt(_net, _top, _flow, _flow_src, _flow_dst, this, _sink, seqno,
            0, _pkt_size, true, last_packet);
	
        p->flow().logTraffic(*p,*this,TrafficLogger::PKT_CREATESEND);
        p->set_ts(eventlist().now());
        _global_rto_count++;
        if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Total RTOs: " << _global_rto_count << endl;
        //p->sendOn();
        //p->sendToNIC();
        sendToNIC(p);
        _packets_sent++;
...
void NdpSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    if (cbb_logging::verbose_enabled()) cbb_logging::debug() <<"At " << timeAsUs(now) << "us RTO " << timeAsUs(_rto) << "us MDEV " << timeAsUs(_mdev) << "us RTT "<< timeAsUs(_rtt) << "us SEQ " << _last_acked / _mss << " CWND "<< _cwnd/_mss << " Flow ID " << str()  << endl;
    /*
    if (_log_me) {
    ...
    */
	...
	if (now > _rtx_timeout) {
	    ...
	    cbb_logging::error() << "late_rtx_timeout: " << _rtx_timeout << " now: " << now << " now+rto: " << now + _rto << " rto: " << _rto << endl;
	    too_early = 0;
	}
...
void NdpSrc::log_rtt(simtime_picosec sent_time) {
	...
	cbb_logging::error() << "Negative RTT: " << rtt << endl;
}
...
NdpSink::NdpSink(networkDevices* net, dynamicTopology* top, NdpPullPacer* pacer, int flow_src, int flow_dst)
    : Logged("ndp_sink"),_cumulative_ack(0), _top(top), _net(net) {
...
void NdpSink::connect(NdpSrc& src)
{
    ...
    // debugging hack
    if (get_id() == LOGSINK) {
        if (cbb_logging::verbose_enabled()) cbb_logging::debug() << "Found sink for " << LOGSINK << "\n";
        _log_me = true;
        _pacer->log_me();
    }
}
...
void NdpSink::receivePacket(Packet& pkt) {
    ...
    if (cbb_logging::verbose_enabled() && p->get_ndpsrc()->get_flowid() == 3469) {cbb_logging::debug() << "sink received a packet, at " << _src->eventlist().now() << endl;}
    ...
    case NDPPULL:
        ...
        assert(pkt.bounced());
        cbb_logging::error() << "Got bounced feedback packet!\n";
...
void NdpSink::send_ack(simtime_picosec ts, NdpPacket::seq_t ackno, NdpPacket::seq_t pacer_no) {
    ...
    ack = NdpAck::newpkt(_net, _top, _src->_flow, _flow_dst, _flow_src, _src, 0,
...
void NdpSink::send_nack(simtime_picosec ts, NdpPacket::seq_t ackno, NdpPacket::seq_t pacer_no) {
    ...
    nack = NdpNack::newpkt(_net, _top, _src->_flow, _flow_dst, _flow_src, _src, 0,
        ackno, _cumulative_ack, _pull_no);
...
{
    // _packet_drain_time = (simtime_picosec)(Packet::data_packet_size() * (pow(10.0,12.0) * 8) / speedFromMbps((uint64_t)_pull_rate*8/1000000))/pull_rate_modifier;
    ...
    : EventSource(event, "ndp_pacer"), _last_pull(0)
{
    ...
    if (!_pull_spacing_cdf) {
        ...
        if (fscanf(f, "%d\n", &_pull_spacing_cdf_count) != 1) {
            fprintf(stderr, "Error reading _pull_spacing_cdf_count from file\n");
            fclose(f);
            exit(EXIT_FAILURE);
        }

        if (cbb_logging::verbose_enabled()) {
            cbb_logging::debug() << "Generating pull spacing from CDF; reading " 
                 << _pull_spacing_cdf_count 
                 << " entries from CDF file " << filename << endl;
        }
        ...
        for (int i = 0; i < _pull_spacing_cdf_count; i++) {
            ...
            if (fscanf(f, "%d %lf\n", &t, &_pull_spacing_cdf[i]) != 2) {
                fprintf(stderr, "Error reading pull spacing data at index %d\n", i);
                fclose(f);
                delete[] _pull_spacing_cdf;
                exit(EXIT_FAILURE);
            }
            // Optional: Ensure `t` matches `i` if required
            // assert(t == i);
        }
...
void NdpPullPacer::sendToNIC(Packet* pkt) {
    ...
    //Queue* nic = top->get_queue_serv_tor(pkt->get_src()); // returns pointer to nic queue
    // Queue* nic = net->get_queue_serv_tor(pkt->get_src());
    PriorityQueue* nic = (PriorityQueue*)net->get_queue_serv_tor(pkt->get_src());
    nic->receivePacket(*pkt); // send this packet to the nic queue
}
...
void NdpPullPacer::sendPacket(Packet* ack, NdpPacket::seq_t rcvd_pacer_no, NdpSink* receiver) {
    ...
    */
    ...
    if (rcvd_pacer_no != 0 && _pacer_no - rcvd_pacer_no < RCV_CWND) {
        ...
        if (_log_me && cbb_logging::verbose_enabled())
            cbb_logging::debug() << "increase_window\n";
    ...
    if (_pull_queue.empty()){
        ...
        if (delta >= drain_time){
            ...
            //ack->sendToNIC();
            //cout << "ackkk\n";
            if (cbb_logging::verbose_enabled() && ack->get_ndpsrc()->get_flowid() == 3469) {cbb_logging::debug() << "pacer sending an ack to NIC, at" << eventlist().now() << endl;}
            sendToNIC(ack);
