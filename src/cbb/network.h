...
typedef enum {IP, TCP, TCPACK, TCPNACK, NDP, NDPACK, NDPNACK, NDPPULL, NDPLITE, NDPLITEACK, NDPLITEPULL, NDPLITERTS, ETH_PAUSE, RLB, CBB_CNT} packet_type;
...
class Packet {
 ...
 public:
    ...
    void set_topology(dynamicTopology* top) {_top = top;} // pass in the pointer to the topology object
    void set_device(networkDevices* net) {_net = net;}
    dynamicTopology* get_topology() {return _top;}
    networkDevices* get_device() {return _net;}
    ...
    void set_current_local_src(int curr_src) {_curr_src = curr_src;}
    int get_current_local_src() {return _curr_src;}

    void set_is_da(int is_da) {_is_da = is_da;}
    int get_is_da() {return _is_da;}

    void set_commit_port(int port) {_commit_port = port;}
    int get_commit_port() {return _commit_port;}
    ...
    virtual inline ControllerSink* get_conSink(){ return NULL; }
    // -------------------
    ...
    bool is_to_controller(){ return _is_to_controller; }
    void to_controller(bool toCon){ _is_to_controller = toCon; }
 protected:
    bool _is_to_controller;
    ...
    int _is_da; // Check type of flows (DA).
