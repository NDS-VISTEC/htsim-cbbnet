...
NdpSrc::NdpSrc(DynExpTopology* top, NdpLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, int flow_src, int flow_dst)
    : EventSource(eventlist,"ndp"), _flow_src(flow_src), _flow_dst(flow_dst), _top(top), _logger(logger), _flow(pktlogger) {
...
void NdpSrc::processAck(const NdpAck& ack) {
    ...
    */
    ...
    // if (cum_ackno >= _flow_size) {delete this; return;}
}
...
NdpSink::NdpSink(DynExpTopology* top, NdpPullPacer* pacer, int flow_src, int flow_dst)
    : Logged("ndp_sink"),_cumulative_ack(0) , _flow_src(flow_src), _flow_dst(flow_dst), _top(top), _total_received(0)
{
    _src = 0;
    _pacer = pacer;
    _nodename = "ndpsink";
...
//     simtime_picosec now = eventlist().now();
//     tcps_t::iterator i;
//     for (i = _tcps.begin(); i!=_tcps.end();) {

//         if ((*i)->_is_finished) {
//             NdpSrc* src = *i;
//             i = _tcps.erase(i);
//             delete src;
//             cout << "src deleted" << endl;
//         }
//         else {
