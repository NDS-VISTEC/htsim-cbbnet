...
void Queue::sendFromQueue(Packet* pkt) {
    ...
    //else {
        if (pkt->get_crthop() < 0) {
            ...
            // cout << "Packet sent from ToR " << pkt->get_crtToR() << " at " << eventlist().now() << endl;
            //}
        }
    
    // cout << "curr hop is " << pkt->get_crthop() << " dst = " << pkt->get_dst() << " last hop = " << pkt->is_lasthop() << endl;
    // cout << "Packet sent from ToR " << pkt->get_crtToR() << " to port " << pkt->get_crtport() << " at " << eventlist().now() << endl;
	...
	if (!_queue[_servicing].empty()) {
	    ...
	    case Q_HI:
	    {
	        ...
	        if (pkt->get_src_ToR() == _top->get_firstToR(pkt->get_dst())) {
	            ...
	            // cout << "in queue slice = " << slice << endl; 
