...
void RlbSrc::startflow() {
    ...
    // cout << "start RLB flow from "<< _flow_src << " to " << _flow_dst << " size = " << _flow_size << " Bytes" << endl;
    // cout << "from ToR "<< _flow_src/_top->no_of_hpr() << " to ToR " << _flow_dst/_top->no_of_hpr() << endl;
...
RlbSink::RlbSink(DynExpTopology* top, EventList &eventlist, int flow_src, int flow_dst)
    : EventSource(eventlist,"rlbsnk"), _flow_src(flow_src), _flow_dst(flow_dst), _top(top), _total_received(0)
{
    _src = 0;
    _nodename = "rlbsink";
    _total_received = 0;
