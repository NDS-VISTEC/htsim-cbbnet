    ...
    //else {
        ...
        if (pkt->is_lasthop()) {
            ...
            if (pkt->get_crtToR() < 0)
                ...
                long unsigned int superslice = (eventlist().now() / top->get_slicetime(3)) %
                    top->get_nsuperslice();
                // next, get the relative time from the beginning of that superslice
                long unsigned int reltime = eventlist().now() - superslice*top->get_slicetime(3) -
                ...
                // cout << "pkt pipe = " << pkt << endl;  
                // cout << "nextToR = " << nextToR << ", slice = " << slice << ", get_crtToR() = " << pkt->get_crtToR() << ", get_crtport = " << pkt->get_crtport() << endl;
...
void UtilMonitor::printAggUtil() {
    ...
    cout << "Num event: " << eventlist().get_event_size() << endl;


    uint64_t B_sum_up = 0;

    for (int hst = 0; hst < _H; hst++) {
        Pipe* pipe = _top->get_pipe_serv_tor(hst);
