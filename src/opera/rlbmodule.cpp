...
// File-level logger instance




RlbModule::RlbModule(DynExpTopology* top, EventList &eventlist, int node)
  : EventSource(eventlist,"rlbmodule"), _top(top), _node(node)
{   
    ...
    cout << "linkrate (in rlb module) is set to " << this->_link_rate << " Bps (" << this->_link_rate*8 << " bps )" <<endl;
...
{
    // cout << "num to push = " << num_to_push << endl;
...
void RlbModule::enqueue_commit(int slice, int current_commit_queue, int Nsenders, const vector<int>& pkts_to_send, const vector<vector<int>>& q_inds, const vector<int>& dst_labels)
{
    ...
    for (int i = 0; i < _max_pkts; i++){
        ...
        while (!found_queue) {
            ...
            // cout << "end_slot_time = " << end_slot_time << endl;
...
void RlbMaster::newMatching() {
    ...
    long unsigned int superslice = (eventlist().now() / _top->get_slicetime(3)) %
    _top->get_nsuperslice();
    // next, get the relative time from the beginning of that superslice
    long unsigned int reltime = eventlist().now() - superslice*_top->get_slicetime(3) -
    ...
    for (int host = 0; host < _H; host++) {
        ...
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < _H; k++){
                ...
                if (_working_queue_sizes[host][j][k] > 0) {
                    // cout << "Line " << __LINE__ << ": check queue " << host << " " << j << " " << k << " = " <<_working_queue_sizes[host][j][k] << endl;
            ...
            //     int sum_q = 0;
            //     for (int k = 0; k < _H; k++)
            //         sum_q += _working_queue_sizes[host][j][k];


            //     if (host%6 == 0 && j==0)
            //         cout << endl << "ToR " << host/6 << " at cycle " << superslice/108 << endl;

            //     if (sum_q > 0)
            //         cout << "host = " << host << ", local = " << j << ": " << sum_q << endl;
            // }
            }
        }
    }
    ...
    for (int crtToR = 0; crtToR < _N; crtToR++) {
        ...
        if (crtToR != dstToR) {
            ...
            for (int j = 0; j < static_cast<int>(src_hosts.size()); j++){
                for (int k = 0; k < static_cast<int>(dst_hosts.size()); k++) {

                    for (int l = 0; l < _H; l++) {
                        int temp = _accepts[src_hosts[j]][dst_hosts[k]][l];
                        if (temp > 0) {
                            // cout << "temp = " << temp << ", from " << src_hosts[j] << " to " << dst_hosts[k] << " " << l << endl; 
...
void RlbMaster::phase1(const vector<int>& src_hosts, const vector<int>& dst_hosts)
{
    ...
    for (int i = 0; i < static_cast<int>(src_hosts.size()); i++) { // sweep senders

        _Nsenders[src_hosts[i]] = 0; // initialize number of sending queues to zero
    
        for (int j = 0; j < static_cast<int>(dst_hosts.size()); j++) { // sweep destinations
    ...
    for (int i = 0; i < static_cast<int>(src_hosts.size()); i++)
        local_pkts_rack[i].resize(dst_hosts.size());

    for (int i = 0; i < static_cast<int>(src_hosts.size()); i++) {
        for (int j = 0; j < static_cast<int>(dst_hosts.size()); j++) {
    ...
    // get vectors of the capacities:
    vector<int> dst_caps;
    for (int i = 0; i < static_cast<int>(dst_hosts.size()); i++)
...
void RlbMaster::phase2(const vector<int>& src_hosts, const vector<int>& dst_hosts)
{
    ...
    // cout << "fixed_pkts = " << fixed_pkts << endl;

    for (int i = 0; i < static_cast<int>(dst_hosts.size()); i++) { // sweep destinations
...
vector<vector<int>> RlbMaster::fairshare2d(vector<vector<int>> input, const vector<int>& cap0, const vector<int>& cap1) {
    ...
    for (int i = 0; i < static_cast<int>(input.size()); i++) {
        ...
        for (int j=0; j < static_cast<int>(input[0].size()); j++)
            sent[i][j] = 0;
    }
    ...
    while (nelem != 0 && iter < maxiter) {
        ...
        for (int i = 0; i < static_cast<int>(input[0].size()); i++) {
            int prev_alloc = 0;
