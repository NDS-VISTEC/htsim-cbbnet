...
// #define DEFAULT_PACKET_SIZE 6000 // MAXIMUM FULL packet size (includes header + payload), Bytes
...
uint64_t getLastColumnValue(const std::string& filename) {
    // Open the file
    ...
    while (file >> std::ws && getline(file, line)) // skip empty lines
        ;
    // getline(file, line);
    stringstream stream(line);
    // Variable to store the last value
    uint64_t lastValue = 0; // Initialize to a sentinel value

    stream >> lastValue;
    stream >> lastValue;
    stream >> lastValue;
    stream >> lastValue;
    // Read the file backwards until a space character is encountered
    // char ch;
    // while (file.tellg() > 0 && (ch = file.get()) != ' ') {
    //     file.seekg(-2, std::ios::cur);
    // }

    // if (!(file >> lastValue)) {
    //     std::cerr << "Error reading the last value from the file" << std::endl;
    //     return -1; // Return a sentinel value to indicate an error
    // }
...
int main(int argc, char **argv) {
    ...
    double pull_rate = 0; // set the pull rate from the command line
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")){
	       ...
	       i++;
            ...
            i++;
        } else if (!strcmp(argv[i],"-link_rate_bps")) {
            link_rate_bps = atof(argv[i+1]);
            _link_rate = link_rate_bps/8;
            i++;
    ...
    // uint64_t endtime = getLastColumnValue(flowfile);
    // eventlist.setEndtime(timeFromNs(endtime)); // in seconds
    ...
    if (link_rate_bps == 0) {
	   fprintf(stderr, "set -linkrate_bps\n");
	   exit(1);
    }
...
// this creates the Expander topology
#ifdef DYNEXP
    DynExpTopology* top = new DynExpTopology(queuesize, NULL, &eventlist, COMPOSITE, topfile);
#endif
    top->set_link_rate(_link_rate);
    top->init_network();
    ...
    if (input.is_open()){
        ...
        while(!input.eof()){
            ...
            if (vtemp[2] < cutoff && vtemp[2] != rlbflow) { // priority flow, sent it over NDP
                ...
                flowpacer->set_pull_rate(_link_rate, pull_rate);
    ...
    cout << "Done" << endl;
    std::cout << "Time taken: " << duration.count()/60 << " hours " << duration.count()%60 << " minutes" << std::endl;
