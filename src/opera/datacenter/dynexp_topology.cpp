...
void DynExpTopology::read_params(string topfile) {
  ...
  if (input.is_open()){
    ...
    int sz = 0;
    int slice = -1; // which topology slice we're in
    while(!input.eof()) {
      ...
      if (vtemp.size() == 1) { // entering the next topology slice
        ...
        for (int i = 2; i < static_cast<int>(vtemp.size()); i++) {
...
Queue* DynExpTopology::alloc_src_queue(DynExpTopology* top, QueueLogger* queueLogger, int node) {
    return new PriorityQueue(top, speedFromMbps((uint64_t)_link_rate*8/1000000), memFromPkt(FEEDER_BUFFER), *eventlist, queueLogger, node);
}

Queue* DynExpTopology::alloc_queue(QueueLogger* queueLogger, mem_b queuesize, int tor, int port) {
    return alloc_queue(queueLogger, _link_rate*8/1000000, queuesize, tor, port);
}
